...
app.post('/api/seed', async (req, res) => {
  ...
  const { sequelize, Client, Bike, WorkOrder, ChecklistItem, WorkOrderItem, WorkOrderStatusHistory } = require('./models');
  ...
  try {
    ...
    const orden1 = await WorkOrder.create({
      bikeId: motos[0].id, pilotName: clientes[0].name, serviceType: 'ALISTAMIENTO',
      status: 'LISTA', hoursUsed: 16, hoursRegistered: 0, total: 150000, entryDate: '2026-04-13',
    });
    ...
    const orden2 = await WorkOrder.create({
      bikeId: motos[2].id, pilotName: clientes[1].name, serviceType: 'MANTENIMIENTO',
      status: 'EN_PROCESO', hoursUsed: 8, hoursRegistered: 0, total: 200000, entryDate: '2026-04-15',
    });
    
    const orden3 = await WorkOrder.create({
      bikeId: motos[4].id, pilotName: clientes[2].name, serviceType: 'REPARACION',
      status: 'RECIBIDA', hoursUsed: 0, hoursRegistered: 150, total: 350000, entryDate: '2026-04-16',
    });
    
    const orden4 = await WorkOrder.create({
      bikeId: motos[5].id, pilotName: clientes[3].name, serviceType: 'ALISTAMIENTO',
      status: 'LISTA', hoursUsed: 12, hoursRegistered: 30, total: 180000, entryDate: '2026-04-10',
    });
    
    const orden5 = await WorkOrder.create({
      bikeId: motos[6].id, pilotName: clientes[4].name, serviceType: 'MANTENIMIENTO',
      status: 'ENTREGADA', hoursUsed: 20, hoursRegistered: 65, total: 220000, entryDate: '2026-04-08',
    });
    ...
    res.json({ ok: true, message: 'Base de datos poblada', counts: { clientes: 5, motos: 7, items: 12, ordenes: 5 } });
  } catch (error) {
